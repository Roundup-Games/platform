...
- **Phase 3 (deferred):** Full Blade + view-contract modernization — wrapper and partials read typed props off `$dashboard` (the `AssembledDashboard` view-model) directly, `toViewProps()` is deleted, and the test suite stops asserting on the flat view-variable contract for the inactive mode. Not done in this refactor. Investigation showed the test suite asserts on the full view-data contract regardless of mode (e.g. `DashboardNewcomerTest` reads `viewData('newcomerWelcome')` on an established-mode user and expects `[]`), and `toViewProps()` emits 16 inactive-mode stub keys specifically to satisfy that contract. The Blade wrapper already branches on `dashboardMode` so the stubs are inert at render time, but removing them is not behavior-preserving — it requires updating the test suite's view-contract assertions in the same change. That belongs in its own change with its own review, not ridden on this refactor. The dead `gamesThisWeek` / `gamesThisWeekCount` shared keys (carry-overs from a removed feature, consumed by no Blade and asserted by no test) were the only safe deletions and are removed here.
