...
class DiscordWebhookClient
{
        ...
        while (true) {
            ...
            if ($response->failed()) {
                ...
                Log::error('Discord API client error', [
                    'endpoint' => $this->sanitizeEndpoint($path),
                    'status' => $status,
    ...
    /**
     * Redact credentials from an endpoint path before it is written to logs.
     *
     * Interaction-webhook paths (`webhooks/{app}/{token}/…`) are
     * self-authenticating: the token segment IS the credential (valid 15 min).
     * Channel-message paths (`channels/…/messages/…`) carry only snowflakes
     * authenticated by the bot header, so they are logged verbatim.
     */
    private function sanitizeEndpoint(string $path): string
    {
        return preg_replace('#^(webhooks/[^/]+/)[^/]+(/.*)?$#', '$1***$2', $path) ?? $path;
    }
}
