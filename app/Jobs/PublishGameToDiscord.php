 ...
 */
class PublishGameToDiscord implements ShouldBeUnique, ShouldQueue
{
    use Dispatchable;
    ...
    /**
     * Hold the unique lock long enough to coalesce a burst of dispatches for
     * the same game (a double-save, a retry racing a fresh dispatch) into one
     * publish — mirrors {@see RefreshDiscordCard}. The publisher is
     * edit-in-place idempotent, so a suppressed dispatch converges on the run
     * that did execute.
     */
    ...
    /**
     * Unique key per game — coalesces rapid dispatches into one publish.
     */
