...
class OrganizerGuilds extends Component
{
     ...
     *
     * Resolves the guild from the organizer's discovered membership set (the
     * intersection of their Discord guilds and roundup-enabled servers) rather
     * than trusting a client-supplied row id directly — so an organizer can
     * only opt in to a guild they are actually a member of.
     */
    public function optIn(string $guildId, DiscordGuildDiscoveryService $discovery): void
    {
        $guild = $this->discoveredGuild($guildId, $discovery);
    ...
    /**
     * Resolve a surfaced guild id to its model ONLY if the organizer is a
     * member of it. Discovery intersects the organizer's Discord membership
     * against roundup-enabled guilds, so a guild that does not appear in the
     * discovered set is one the organizer is not in — and must not be able to
     * opt in to. This closes the write-side gap where optIn trusted a
     * client-supplied guild row id.
     */
    private function discoveredGuild(string $guildId, DiscordGuildDiscoveryService $discovery): ?DiscordGuild
    {
        foreach ($discovery->discoverFor($this->organizer()) as $surfaced) {
            if ($surfaced->guild->id === $guildId) {
                return $surfaced->guild;
            }
        }
