    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 space-y-10">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex flex-wrap items-center gap-2 mb-1">
                                    <h3 class="text-base font-medium text-on-surface truncate">
                                        <a href="{{ route('campaigns.detail', $campaign->id) }}" wire:navigate class="hover:text-secondary transition-colors">
                                            {{ $campaign->name }}
                                        </a>
                                    </h3>
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                @if($campaign->status === 'active')
                                    <button wire:click="cancelCampaign('{{ $campaign->id }}')"
                                        ...
                                        {{ __('campaigns.action_cancel_campaign') }}
                                    </button>
                                    <button wire:click="completeCampaign('{{ $campaign->id }}')"
                                        ...
                                        <span class="material-symbols-outlined text-base" aria-hidden="true">check_circle</span>
                                        {{ __('campaigns.action_complete_campaign') }}
                                    </button>
                                @endif
                    ...
                    <div class="bg-surface-container-low rounded-xl shadow-ambient p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4 border-l-4 border-primary">
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            <button wire:click="acceptInvitation('{{ $invitation->id }}')"
                                ...
                                {{ __('campaigns.action_accept_invitation') }}
                            </button>
                            <button wire:click="declineInvitation('{{ $invitation->id }}')"
                                    class="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm font-medium text-error hover:bg-error/10 transition-colors">
                                <span class="material-symbols-outlined text-base" aria-hidden="true">close</span>
                                {{ __('campaigns.action_decline_invitation') }}
                            </button>
                        </div>
                    </div>
                @endforeach
        ...
        <section>
            <div class="flex items-center gap-3 mb-4">
                ...
                <h2 class="text-xl font-heading font-semibold text-on-surface">{{ __('campaigns.heading_community') }}</h2>
                <div class="flex-1 h-px bg-outline-variant/30"></div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row gap-3 mb-4">
                ...
                <select wire:model.live="game_system_id" aria-label="{{ __('discovery.content_all_systems') }}"
            ...
            <div class="flex flex-wrap gap-3 mb-4">
                <select wire:model.live="experience_level" aria-label="{{ __('discovery.content_all_levels') }}"
                ...
                <select wire:model.live="language" aria-label="{{ __('discovery.content_all_languages') }}"
                        class="bg-surface-container-high border border-transparent rounded-lg text-on-surface text-sm shadow-sm focus:border-secondary/20 focus:ring-2 focus:ring-secondary/20">
            ...
            <div class="flex items-center gap-3 mb-4">
                ...
                <input type="number" min="1" max="5" step="0.5" wire:model.live="complexity_min" placeholder="{{ __('common.field_min') }}" aria-label="Minimum complexity"
                ...
                <input type="number" min="1" max="5" step="0.5" wire:model.live="complexity_max" placeholder="{{ __('common.field_max') }}" aria-label="Maximum complexity"
                       class="w-20 bg-surface-container-high border border-transparent rounded-lg text-on-surface text-sm text-center shadow-sm focus:border-secondary/20 focus:ring-2 focus:ring-secondary/20" />
            </div>
                ...
                <div class="flex items-center gap-2 flex-wrap mb-4">
                    ...
                    @if($recurrence)
                        <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-surface-container text-on-surface-variant">
                            {{ __('campaigns.content_' . $recurrence) }}
                        </span>
                    @endif
                    @if($price)
                        <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-secondary-container text-on-secondary-container">
            ...
            {{-- Community Campaigns Grid --}}
            @if($communityCampaigns->count())
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($communityCampaigns as $campaign)
                        <a href="{{ route('campaigns.detail', $campaign->id) }}" wire:navigate class="block bg-surface-container-low rounded-xl shadow-ambient hover:shadow-md transition-shadow overflow-hidden group">
                            ...
                            <div class="p-5">
                                ...
                                {{-- Game System + Visibility + Experience badges --}}
                                ...
                                @if(isset($campaign->sessions_count))
                                    <p class="mt-1 text-sm text-on-surface-variant flex items-center gap-1">
                                        <span class="material-symbols-outlined text-base" aria-hidden="true">event_note</span>
                                        {{ $campaign->sessions_count }} session(s)
                                    </p>
                                @endif
                                ...
                                <div class="mt-3 flex items-center gap-3 text-xs text-on-surface-variant">
                                    <span class="flex items-center gap-1">
                                        <span class="material-symbols-outlined text-sm" aria-hidden="true">group</span>
                                        {{ $campaign->participants_count }}/{{ $campaign->max_players ?? '∞' }}
                                    </span>
                                </div>
                            </div>
                        </a>
            ...
            @else
                <div class="text-center py-16 bg-surface-container-low rounded-xl shadow-ambient">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-on-surface">{{ __('campaigns.content_no_community_campaigns') }}</h3>
