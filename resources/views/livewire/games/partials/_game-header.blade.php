...
<section class="relative bg-primary text-on-primary overflow-hidden min-h-[200px] sm:min-h-[260px]">
    ...
    <div class="relative max-w-5xl mx-auto px-4 sm:px-6 py-10 sm:py-14 lg:py-16">
        ...
        {{-- Campaign link --}}
        @if($game->campaign)
            <a href="{{ route('campaigns.detail', ['locale' => app()->getLocale(), 'id' => $game->campaign]) }}" wire:navigate class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-on-primary/20 text-on-primary hover:bg-on-primary/30 transition-colors mb-3">
