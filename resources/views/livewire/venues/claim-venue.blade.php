    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="mb-6 sm:mb-8">
            <div class="flex items-center gap-3 mb-1">
                ...
                <h1 class="text-2xl font-heading font-bold tracking-tight text-on-surface">{{ __('venues.heading_claim_venue') }}</h1>
            </div>
            <p class="ml-8 sm:ml-9 text-sm text-on-surface-variant">{{ __('venues.content_claim_subtitle', ['name' => $location->name]) }}</p>
        </div>
        ...
        @if(session()->has('success'))
            <div class="mb-6 bg-primary-container/50 border border-primary/20 rounded-xl p-4 flex items-start gap-3">
                ...
                <div>
                    ...
                    @endif
                    <a href="{{ route('venues.detail', ['locale' => app()->getLocale(), 'slug' => $location->slug]) }}" wire:navigate class="mt-2 inline-block text-sm text-primary hover:underline">{{ __('venues.action_back_to_venue') }}</a>
                </div>
            </div>
        @endif
        ...
        <form wire:submit="submit" class="space-y-6">
            ...
            <section class="bg-surface-container-lowest rounded-xl shadow-ambient p-5 sm:p-6">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <h2 class="text-base font-semibold text-on-surface">{{ __('venues.field_justification') }}</h2>
                </div>

                <div class="space-y-4">
                    {{-- Justification (required) --}}
                    <div>
                        <label for="claim-justification" class="block text-sm font-medium text-on-surface mb-1">{{ __('venues.field_justification') }} <span class="text-error">*</span></label>
                        <textarea id="claim-justification"
                                  wire:model="justification"
                                  rows="4"
                                  placeholder="{{ __('venues.placeholder_justification') }}"
            ...
            <section class="bg-surface-container-lowest rounded-xl shadow-ambient p-5 sm:p-6">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <h2 class="text-base font-semibold text-on-surface">{{ __('venues.heading_optional_proof') }}</h2>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    {{-- Contact email --}}
                    <div>
                        <label for="claim-contact-email" class="block text-sm font-medium text-on-surface mb-1">{{ __('venues.field_contact_email') }}</label>
                        <input type="email"
                               id="claim-contact-email"
                               wire:model="contact_email"
                               placeholder="you@example.com"
                               class="w-full rounded-lg bg-surface-container-high border border-transparent px-4 py-2.5 text-on-surface placeholder:text-on-surface-variant/50 focus:border-secondary/20 focus:ring-1 focus:ring-secondary/20 transition-colors" />
                        <p class="mt-1 text-xs text-on-surface-variant">{{ __('venues.hint_contact_email') }}</p>
