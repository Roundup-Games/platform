    ...
    <div class="max-w-2xl mx-auto space-y-8">
        ...
        <div>
            <div class="flex items-center gap-3 mb-1">
                ...
                <h1 class="text-2xl font-heading font-bold tracking-tight text-on-surface">{{ __('Manage Team') }}</h1>
            </div>
            <p class="ml-8 text-sm text-on-surface-variant">{!! __('Update settings for :name', ['name' => '<strong>' . e($team->name) . '</strong>']) !!}</p>
        </div>
        ...
        @if($saved)
            <div x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)"
                ...
                <p class="text-sm text-on-secondary-container flex items-center gap-2">
                    ...
                    {{ __('Team settings saved successfully.') }}
                </p>
            </div>
        @endif
        ...
        <section class="bg-surface-container-lowest rounded-xl shadow-ambient p-6">
            <h2 class="text-lg font-heading font-semibold text-on-surface tracking-tight mb-4">{{ __('Team Information') }}</h2>
        ...
        <section class="bg-surface-container-lowest rounded-xl shadow-ambient p-6 border-l-4 border-error">
            <h2 class="text-lg font-heading font-semibold text-error mb-1 tracking-tight">{{ __('Danger Zone') }}</h2>
            <p class="text-sm text-on-surface-variant mb-4">{{ __('Permanently delete this team. This cannot be undone.') }}</p>
            <button onclick="confirm('{{ __("Are you sure you want to delete this team?") }}') || event.preventDefault()"
