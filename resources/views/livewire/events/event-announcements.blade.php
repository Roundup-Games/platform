<div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-surface-container-low rounded-xl shadow-ambient p-4 text-center">
            ...
            <p class="text-xs text-on-surface-variant tracking-wide">Published</p>
        </div>
        <div class="bg-surface-container-low rounded-xl shadow-ambient p-4 text-center">
            <p class="text-2xl font-bold text-tertiary">{{ $this->counts['draft'] }}</p>
            <p class="text-xs text-on-surface-variant tracking-wide">Drafts</p>
        </div>
        <div class="bg-surface-container-low rounded-xl shadow-ambient p-4 text-center">
            <p class="text-2xl font-bold text-primary">{{ $this->counts['pinned'] }}</p>
            <p class="text-xs text-on-surface-variant tracking-wide">Pinned</p>
        </div>
    </div>
    ...
    @if($showForm)
        <div class="bg-surface-container-low rounded-xl shadow-ambient p-6 mb-6">
            ...
            <div class="space-y-4">
                <div>
                    <label for="announcement-title" class="block text-sm font-medium text-on-surface-variant mb-1">Title *</label>
                ...
                <div>
                    <label for="announcement-content" class="block text-sm font-medium text-on-surface-variant mb-1">Content *</label>
                ...
                <div class="flex items-center gap-6">
                    <label class="flex items-center gap-2 cursor-pointer">
                        ...
                        <span class="text-sm text-on-surface-variant">Publish immediately</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" wire:model="is_pinned" class="rounded border-outline text-primary focus:ring-primary/20" />
                        <span class="text-sm text-on-surface-variant">Pin to top</span>
                    </label>
                </div>
        ...
        <div class="divide-y divide-outline-variant/30">
            @if($this->announcements->isEmpty())
                <div class="text-center py-12">
                    <p class="text-on-surface-variant">No announcements yet.</p>
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2 mb-1">
                                    ...
                                    @if($announcement->is_published)
                                        <span class="text-xs px-1.5 py-0.5 rounded bg-secondary-container text-on-secondary-container">Published</span>
                                    @else
                                        <span class="text-xs px-1.5 py-0.5 rounded bg-tertiary/10 text-on-tertiary-container">Draft</span>
                                    @endif
                                </div>
