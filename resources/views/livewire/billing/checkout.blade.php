<div class="py-8">
    <div class="max-w-2xl mx-auto space-y-8">
        {{-- Page Header --}}
        <div>
            <h1 class="text-2xl font-heading font-bold tracking-tight text-on-surface">Checkout</h1>
        </div>
        ...
        <section class="bg-surface-container-lowest rounded-xl shadow-ambient p-6">
            <h2 class="text-lg font-heading font-semibold tracking-tight text-on-surface mb-4">Order Summary</h2>
            ...
            @if($mode === 'subscription' && $membershipType)
                <div class="space-y-3">
                    ...
                    @endif
                    <p class="text-xs text-on-surface-variant/70">Duration: {{ $membershipType->duration_months }} month{{ $membershipType->duration_months > 1 ? 's' : '' }}</p>
                </div>
            @elseif($mode === 'one-time')
                <div class="space-y-3">
                    <div class="flex justify-between items-center">
                        <span class="text-on-surface font-medium">Event Registration</span>
                    </div>
                    <p class="text-sm text-on-surface-variant">One-time payment for event registration.</p>
                </div>
            @endif
