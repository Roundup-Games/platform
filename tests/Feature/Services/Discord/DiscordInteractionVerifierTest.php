...
class DiscordInteractionVerifierTest extends TestCase
{
    ...
    #[Test]
    public function verify_returns_false_for_a_correctly_signed_but_stale_timestamp()
    {
        // A valid signature is proof of authenticity, NOT freshness: a
        // captured (body, signature, timestamp) triple must not verify
        // forever. A correctly-signed body with a timestamp far outside the
        // tolerance window is rejected before the signature matters.
        [$verifier, , $sign] = $this->makeVerifierAndSigner();

        $body = '{"type":1}';
        $staleTimestamp = (string) (time() - 3600); // 1 hour ago
        $signature = $sign($body, $staleTimestamp);

        $this->assertFalse($verifier->verify($body, $signature, $staleTimestamp));
    }

    #[Test]
    public function verify_returns_false_for_a_correctly_signed_future_dated_timestamp()
    {
        // Symmetric protection against clock-skew abuse.
        [$verifier, , $sign] = $this->makeVerifierAndSigner();

        $body = '{"type":1}';
        $futureTimestamp = (string) (time() + 3600); // 1 hour ahead
        $signature = $sign($body, $futureTimestamp);

        $this->assertFalse($verifier->verify($body, $signature, $futureTimestamp));
    }
}
