...
// Regression test (T05): venue-reported reviews (reviewable_type=Location) must
// flow through the UNCHANGED generic ReportReview → Escalated Safety ticket →
// admin-moderation pipeline, and the admin hide/remove must recompute the venue
// aggregate via the ReviewObserver's Location branch (T03). This mirrors
// EscalatedReviewReportTest scenario-for-scenario. If any venue-specific code
// change becomes necessary in ReportReview/TicketPayloadRenderer/ViewTicket,
// that contradicts MEM527/D083 and is a blocker.
...
/**
 * Helper: create a verified *commercial* venue with a published, attended venue
 * review (reviewable_type=Location, gm_profile_id=null).
 *
 ...
 * Reuses the generic ReportReview component (no venue-specific code).
 */
function reportVenueReviewAndGetTicket(Review $review, User $reporter, string $reason = 'harassment'): Ticket
{
...
it('creates an escalated ticket in Safety department with high priority when a venue review is reported', function () {
    ...
    $ticket = reportVenueReviewAndGetTicket($review, $reporter, 'inappropriate');
...
// ────────────────────────────────────────────────────────────────────────
// 2. Ticket metadata contains review_id, author, reporter, reason + tag
// ────────────────────────────────────────────────────────────────────────

it('stores complete venue review context in ticket metadata', function () {
    ['review' => $review, 'reviewer' => $reviewAuthor] = createVenueReviewSetup();
    $reporter = User::factory()->create(['profile_complete' => true]);

    $ticket = reportVenueReviewAndGetTicket($review, $reporter, 'spam');
...
it('dismiss report closes ticket and keeps venue review published', function () {
    ['review' => $review] = createVenueReviewSetup();
    $reporter = User::factory()->create(['profile_complete' => true]);

    $ticket = reportVenueReviewAndGetTicket($review, $reporter);
    ...
    expect($review->body)->toBe('Offensive venue review content'); // content unchanged
});
...
it('remove venue review closes ticket and sets review status to hidden', function () {
    ['review' => $review] = createVenueReviewSetup();
    $reporter = User::factory()->create(['profile_complete' => true]);
...
// ────────────────────────────────────────────────────────────────────────
// 6. ReviewReported notification still fires for venue reports
// ────────────────────────────────────────────────────────────────────────

it('sends ReviewReported notification to global admins when a venue review is reported', function () {
    ...
    reportVenueReviewAndGetTicket($review, $reporter, 'harassment');
...
// ────────────────────────────────────────────────────────────────────────
// 7. ReviewObserver venue aggregate recalculation after status changes
// ────────────────────────────────────────────────────────────────────────
...
it('ReviewObserver recalculates venue aggregates when review status changes from reported to hidden', function () {
    ...
    // Establish baseline: one published venue review
    app(ReviewAggregateService::class)->updateLocationAggregates($venue);
    expect($venue->fresh()->review_count)->toBe(1);
    expect($venue->fresh()->average_rating)->not->toBeNull();

    // Report the review — status goes to 'reported' → observer fires venue branch
    reportVenueReviewAndGetTicket($review, $reporter);

    // Refresh the review model (modified inside Livewire sub-request)
    $review->refresh();

    // reported reviews don't count as published, so venue count should be 0
    expect($venue->fresh()->review_count)->toBe(0);
    ...
    $review->update(['status' => 'hidden']);
    expect($venue->fresh()->review_count)->toBe(0);
    expect($venue->fresh()->average_rating)->toBeNull();
});
...
it('auto-escalation rule fires on aged safety venue review report ticket', function () {
    ['review' => $review] = createVenueReviewSetup();
    $reporter = User::factory()->create(['profile_complete' => true]);
