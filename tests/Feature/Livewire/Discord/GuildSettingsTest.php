...
class GuildSettingsTest extends TestCase
{
    ...
    public function install_callback_creates_discord_guild_with_owner_user_id()
    {
        ...
        $this->installCallback($landlord)->assertRedirect();
    ...
    public function install_callback_redirects_to_guild_settings_page()
    {
        ...
        $this->installCallback($landlord)
            ->assertRedirect('/en/discord/guilds/'.self::GUILD_SNOWFLAKE);
    }
    ...
    public function onboarding_message_posts_to_system_channel_on_install()
    {
        ...
        $this->installCallback($landlord, fakes: [
            'discord.com/api/oauth2/token' => Http::response(['access_token' => 'user-bearer'], 200),
            self::BASE_URL.'/users/@me/guilds' => Http::response([
                ['id' => self::GUILD_SNOWFLAKE, 'owner' => true, 'permissions' => '0'],
            ], 200),
    ...
    public function reinstalling_updates_existing_guild_name_and_owner()
    {
        ...
        $this->installCallback($newOwner, fakes: [
            'discord.com/api/oauth2/token' => Http::response(['access_token' => 'user-bearer'], 200),
            self::BASE_URL.'/users/@me/guilds' => Http::response([
                ['id' => self::GUILD_SNOWFLAKE, 'owner' => true, 'permissions' => '0'],
            ], 200),
     ...
     */
    /**
     * Perform the install callback as $landlord with a valid anti-CSRF state
     * session (mirrors what the redirect route stores) and the guild_id on the
     * callback URL (Discord's real contract). $fakes overrides the default
     * install Http fakes.
     */
    private function installCallback(User $landlord, array $fakes = [], array $query = []): TestResponse
    {
        $state = 'valid-install-state';
        ...
        $defaults = [
            ...
            'discord.com/api/oauth2/token' => Http::response(['access_token' => 'user-bearer'], 200),
            // The installer owns the claimed guild.
            self::BASE_URL.'/users/@me/guilds' => Http::response([
                ['id' => self::GUILD_SNOWFLAKE, 'owner' => true, 'permissions' => '0'],
            ], 200),
            self::BASE_URL.'/guilds/'.self::GUILD_SNOWFLAKE => Http::response([
        ...
        Http::fake(empty($fakes) ? $defaults : $fakes);

        return $this->actingAs($landlord)
            ->withSession(['discord_install_state' => $state])
            ->get('/discord/install/callback?'.http_build_query(array_merge([
                'code' => 'test-auth-code',
                'guild_id' => self::GUILD_SNOWFLAKE,
                'state' => $state,
            ], $query)));
